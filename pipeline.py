    ...
    for pair in sample_list:
        ...
        with open("/".join([script_dir,samp+".sh"]),"w") as bash:
            ...
            if not os.path.exists(align_dir):
                os.mkdir(align_dir)
            Alignment = "%s mem -t 2 %s %s %s > %s"%(pod['BWA_Path'], pod['Reference_Genome'],"/".join([pod['Fastq_location'],R1]),"/".join([pod['Fastq_location'],R2]), "/".join([align_dir,samp+".sam"]))
            ...
            if not os.path.exists(sort_dir):
                os.mkdir(sort_dir)
            Sorting = "%s -Xmx8g -jar %s SortSam INPUT=%s OUTPUT=%s SORT_ORDER=coordinate CREATE_INDEX=TRUE VALIDATION_STRINGENCY=LENIENT"%(pod['JAVA_Location'], pod['Picard.jar'],"/".join([align_dir,samp+".sam"]), "/".join([sort_dir,samp+".bam"]))
            bash.write(Sorting)
            bash.write("\nwait\n")
            ## HSMETRICS
            ...
            if not os.path.exists(hsmet_dir):
                os.mkdir(hsmet_dir)
            hsmet = "%s -Xmx8g -jar %s CalculateHsMetrics I=%s O=%s R=%s BAIT_INTERVALS=%s TARGET_INTERVALS=%s \nwait"%(pod['JAVA_Location'], pod['Picard.jar'],"/".join([sort_dir,samp+".bam"]),"/".join([hsmet_dir,samp+".txt"]),pod['Reference_Genome'],BAIT_File,BAIT_File)
            bash.write(hsmet)
            bash.write("\n")
            bash.write("tail -n +7 %s > %s"%("/".join([hsmet_dir,samp+".txt"]),"/".join([hsmet_dir,samp+"Metrics.txt"])))
            bash.write("\nwait\n")
            ...
            Addrep = "%s -Xmx8g -jar %s AddOrReplaceReadGroups INPUT=%s OUTPUT=%s RGID=id RGLB=library RGPL=ILLUMINA RGPU=machine RGSM=%s CREATE_INDEX=TRUE VALIDATION_STRINGENCY=LENIENT"%(pod['JAVA_Location'], pod['Picard.jar'],"/".join([sort_dir,samp+".bam"]),"/".join([addreplacegrps_dir,samp+".bam"]),samp)
            bash.write(Addrep)
            bash.write("\nwait\n")
